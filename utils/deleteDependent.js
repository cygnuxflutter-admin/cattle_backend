...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const COWFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const COWCnt = await dbService.deleteMany(COW,COWFilter);
...
const deleteCOW = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(COW,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const countUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const COWFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const COWCnt =  await dbService.count(COW,COWFilter);
...
const countCOW = async (filter) =>{
  try {
    const COWCnt =  await dbService.count(COW,filter);
    return { COW : COWCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const COWFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const COWCnt = await dbService.updateMany(COW,COWFilter,updateBody);
...
const softDeleteCOW = async (filter,updateBody) =>{  
  try {
    const COWCnt =  await dbService.updateMany(COW,filter);
    return { COW : COWCnt };
  } catch (error){
    throw new Error(error.message);
  }
